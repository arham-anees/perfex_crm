...
class Statuses extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Leadevo_status_model');
    }

    public function index()
    {
        // $data['subjects'] = $this.Leadevo_status_model->get_all();
        $this->load->view('setup/statuses');
    }
    ...
    public function delete()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            $deleted = $this->Leadevo_status_model->delete($status_id);
            ...
            redirect(admin_url('/leadevo/prospects_/statuses'));
        } else {
    ...
    public function create()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if (empty($name)) {
                ...
                redirect('/admin/leadevo/prospects/statuses');
            }

            // Process the data (e.g., insert into the database)
            $inserted = $this->Leadevo_status_model->create($name, "");
    ...
    public function update()
    {
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if (empty($status_id) || empty($name)) {
                ...
                redirect(admin_url('/leadevo/prospects/statuses'));
            }

            $updated = $this->Leadevo_status_model->update($status_id, $name);
