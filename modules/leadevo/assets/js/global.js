...
function appointmentGlobalStaffEditModal(el, staff = null) {
    ...
    $("#modal_wrapper").load(`${admin_url}leadsGen/appointments/modal_internal_crm`, {
        slug: 'create'
    }, function (res) {
 ...
 * leadsGen
 */
function appointmentGloballeadsGenContactsModalNew(el) {
    let type = $(el).attr('data-type');

    createModalWrapper();
    $("#modal_wrapper").load(`${admin_url}leadsGen/appointments/modal_leadsGen_contacts_crm`, {
        user_id: $(el).attr('id'),
        type: type
    }, function (res) {
        preventDoubleClicksModal();
 ...
 * Mutual funcions
 */
$('body').on('submit', '#appointment-internal-crm-form, #appointment-leadsGen-contacts-crm-form, #appointment-contacts-crm-form', function (e) {
    e.preventDefault();
});
