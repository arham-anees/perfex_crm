...
function on_invoice_status_changed($data)
{
    ...
    $invoice = $CI->Client_invoices_model->get($invoice_id);
    if (!$invoice) {
        return false;
    }

    // check if invoice is of cart of campaign
    $campaign = $CI->Campaigns_model->get_by_invoice($invoice_id);
    if ($campaign) {
    ...
    if (count($car_prospects) > 0) {
        $CI->Prospects_model->deliver_prospects_cart($invoice, $car_prospects);
        return;
    }
...
function update_client_invoice_status($id, $force_update = false, $prevent_logging = false)
{
    $CI = &get_instance();

    $CI->load->model('client_invoices_model');
    $invoice = $CI->client_invoices_model->get($id);
    ...
    if ($CI->db->affected_rows() > 0) {
        ...
        $CI->client_invoices_model->log_invoice_activity($invoice->id, 'invoice_activity_status_updated', false, $additional_activity);
